function get_incidents(pat) {
  var url =
    "https://api.pagerduty.com/incidents?total=false&time_zone=UTC&statuses%5B%5D=triggered";
  ...
  xhttp.setRequestHeader("authorization", "Token token=" + pat);
  xhttp.send();
}

function get_oncalls(pat) {
  var url =
    "https://api.pagerduty.com/oncalls?total=false&time_zone=UTC&escalation_policy_ids%5B%5D=PLEBQTR";
  var xhttp = new XMLHttpRequest();
  xhttp.withCredentials = true;
  xhttp.open("GET", url, true);
  xhttp.onreadystatechange = function () {
    if (xhttp.readyState == 4) {
      var result = JSON.parse(xhttp.responseText);
      var oncalls = result.oncalls.sort(
        (a, b) => a.escalation_level - b.escalation_level
      );
...
chrome.storage.sync.get(["pdPat"], function (result) {
  if (result.pdPat) {
    get_incidents(result.pdPat);
    get_oncalls(result.pdPat);
  }
});
